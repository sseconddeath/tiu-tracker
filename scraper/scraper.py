"""
TIU Scraper v7 — stores BOTH full list AND consent list from the actual API.
No client-side filtering — lists match TIU site exactly.
"""
...
class TIUScraper:
    ...
    def run(self, institute_ids=None, eduform_ids=None, direction_ids=None):
        ...
        r = self.s.get(BASE, timeout=30); r.raise_for_status()
    ...
    def _scrape(self, org, ef, di):
        ...
        for sp in specs:
            ...
            if full or consent:
                ...
                log.info("     full:%d consent:%d seats:%d/%d",
                         len(full),len(consent),seats["budget"],seats["total"])
