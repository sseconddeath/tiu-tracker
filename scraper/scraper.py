...
def now_msk(): return datetime.now(MSK).strftime('%d.%m.%Y %H:%M МСК')
...
class TIUScraper:
    ...
    def run(self,institute_ids=None,eduform_ids=None,direction_ids=None):
        ...
        for attempt in range(1,6):
            try:
                ...
                r=self.s.get(BASE,timeout=60);r.raise_for_status();break
            except Exception as e:
                log.warning("  Failed: %s",str(e)[:60])
                if attempt==5:raise
                _time.sleep(attempt*15)
    ...
    def save(self):
        data={"scraped_at":now_msk(),"total_lists":len(self.results),
